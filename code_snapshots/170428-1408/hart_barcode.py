...
def hart_barcode(image,x,y,w,h):
    ...
    crop.save("/tmp/barcode_bars.jpg")
    # 1) scan down all lines, deciding white or black
    firsttime = True
    lastwhite = True
    whitecount = 0
    blackcount = 0
    for y in range(int(crop.size[1]-1),0,-1):
        linecroplist = (0,y,int(crop.size[0]),y+1)
    ...
    # convert wide -->True, narrow-->False
    whites_list = map(lambda el: el >= wavg, whites_list)
    # first two whites, first two blacks should be narrow (False)
    if (blacks_list[0] 
            or blacks_list[1] 
            or whites_list[0] 
            or whites_list[1]):
        logging.getLogger('').debug("Problem with bar code: not finding start group. Bavg %d Wavg %d" % (bavg,wavg))
        logging.getLogger('').debug("Blacks %s\nwhites %s" % (blacks_list, whites_list))
