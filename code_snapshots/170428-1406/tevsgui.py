    ...
    def __init__( self, dbc, logger ):
        ...
        try:
            ...
            try:
                ...
                util.writeto(os.path.join(const.root,"nexttoscan.txt"),"1\n")
                self.logger.warning("Created %s with value 1" % (nextscanfile,))
                self.next_scan_number_entry.set_text("1")
            except Exception, e:
                print e

        nexttoprocessfile = "%s/nexttoprocess.txt" % (util.root(),)
        try:
            ...
            os.mknod(os.path.join(const.root,"nexttoprocess.txt"))
            util.writeto(os.path.join(const.root,"nexttoprocess.txt"),"1\n")
