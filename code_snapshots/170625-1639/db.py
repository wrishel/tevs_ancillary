...
class PostgresDB(object):
    ...
    def insert_new(self, ballot):
        ...
        name0 = self.side_list[0].image_filename
        try:
            name1 = self.side_list[1].image_filename
        except IndexError:
            name1 = None
        cur.execute(_pg_mk, (self.layout_id, 
                             name0, 
                             name1, 
                             ballot.precinct_id, 
                             ballot.party_id) )
        ...
        for vop in ballot.results:
            try:
                cur.execute(_pg_ins, (
                        ...
                        vop.crop_bbox[0],
                        vop.crop_bbox[1],
                        vop.crop_bbox[0],#duplicates for now
                        vop.crop_bbox[1],#duplicates for now
    ...
    def insert(self, ballot):
        ...
        cur.execute(_pg_mk, (search_key, name1, name2, precinct))
        sql_ret = cur.fetchall()
...
_pg_mk = """INSERT INTO ballots (
            ...
            file2,
            precinct
        ) VALUES (now(), %s, %s, %s, %s) RETURNING ballot_id ;"""
