...
def lines_connect(image,a,b,allowed_misses=1):
    ...
    for x in range(x1,x2,1):
        ...
        pabove = image.getpixel((x,y-1))
        pbelow = image.getpixel((x,y+1))
        if (p[0] >= const.line_exit_threshold
            and pabove[0] >= const.line_exit_threshold
            and pbelow[0] >= const.line_exit_threshold):
            problem_count += 1
...
def scan_strips_for_horiz_line_y(image,dpi,starting_x_offset, starting_y_offset=150,height_to_scan=300,top=True):
    ...
    potential_lines = [[],[]]
    retval = starting_y_offset
    starting_x_offset= int(starting_x_offset)
    lastred1 = 255
    lastred2 = 255
    one_sixth = dpi/6
    one_sixtieth = dpi/60
    if top:
        ...
        starty = int(image.size[1] - starting_y_offset)
        endy = int(image.size[1] - starting_y_offset - height_to_scan)
        if endy < 0: endy = 0
        incr = -1
    ...
    for y in range(starty,endy,incr):
        ...
        # be substantially darker than the area just before; how substantial
        # will be a tuned value, trying 32 for starters
        diff1 = abs(thisred1-lastred1)
        diff2 = abs(thisred2-lastred2)
        # We encountered several situations where thisred at a line edge
        # was very slightly above 128, 
        # the standard const.line_darkness_threshold
        # and therefore take thisred * 0.9 rather than thisred
        # mjt 7/4/2012 !!!
        if (abs(thisred1-lastred1) > 32 
            and int(thisred1*.9) <= const.line_darkness_threshold):
            potential_lines[0].append((y1+y2)/2)
            found1 = True
            if ( (abs(thisred1-lastred1) > 32) 
                 and (thisred1 > const.line_darkness_threshold)):
                     pass#rint "Note: line utility flagging line with intensity slightly above threshold."
        lastred1 = thisred1
        if (abs(thisred2-lastred2) > 32 
            and int(thisred2*.9) <= const.line_darkness_threshold):
            potential_lines[1].append((y1+y2)/2)
            found2 = True
            if ( (abs(thisred2-lastred2) > 32) 
        ...
        lastred2 = thisred2
    found = False
    if len(potential_lines[0])==0 and len(potential_lines[1])>0:
        retval = potential_lines[1][0]
        found = True
    if len(potential_lines[0])>0 and len(potential_lines[1])==0:
        retval = potential_lines[0][0]
        found = True
    ...
    if not found:
        ...
        raise LineUtilException(potential_lines)
    return retval
