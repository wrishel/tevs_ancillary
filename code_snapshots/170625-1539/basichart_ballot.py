...
from basichart_ballot_side import BasichartBallotSide

class BasichartBallot(Ballot):
    ...
    def __init__(self, image_filenames, extensions):
        super(BasichartBallot, self).__init__(image_filenames, extensions)
        def add_page(number, fname):
            nbs = BasichartBallotSide(
                ballot=self,
                dpi=const.dpi,
...
if __name__ == "__main__":
    print "Test by setting the configuration file to brand Basic"
