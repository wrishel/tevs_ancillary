...
def Template_to_XML(template): #XXX needs to be updated for jurisdictions
    ...
    for contest in template.contests: #XXX should be jurisdiction
        ...
        for choice in contest.choices:
            # insert choice only if choice y > (contest_y + 25)
            if choice.y <= (contest.y + 25):
                print "Skipping choice %s at %s due to contest at %s" % (
                    choice.description,
                    choice.y,
                    contest.y)
                continue
...
def Template_from_XML(xml): #XXX needs to be updated for jurisdictions
    ...
    for contest in tag(side, "Contest"):
        ...
        #only save contests if they've had choices appended
        if choice_count >= 2:
            contests.append(cur)
...
if __name__ == "__main__":
    if len(sys.argv)<2:
        print "usage: python template.py templatefile"
        sys.exit(0)
    # open template file
    startdir = os.path.dirname(sys.argv[1])
    print "Input file path",startdir
    newdir = startdir+"/cleaned"
    try:
        os.mkdir(newdir)
    except:
        pass
    ...
    print base
    outname = os.path.join(newdir,base)
    print "Output file path",outname
    tfile = open(sys.argv[1],"r")
    # read into tree
    tstring = tfile.read()
    tfile.close()
    t = Template_from_XML(tstring)
    # re-write from tree
    newstring = Template_to_XML(t)
    # save template.new file
