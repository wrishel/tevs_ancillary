...
class QueryRequestor(Requestor):
    def __init__(self,stdout_cb=None,stderr_cb=None,hup_cb=None,query=None,user=None,database=None,retfile=None,stdout_cb_data=None, hup_cb_data=None):
        self.user = user
        self.database = database
        # put the data (request) in a command file and pass that to 
        # a small program which pickles results and calls us back
        # (all of our packages programs should be in same directory)
        path = os.path.split(__file__)[0]
        runpath = os.path.join(path,"tevsgui_db.py")
